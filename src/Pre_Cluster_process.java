...
public class Pre_Cluster_process {
	private String input_path = new String(); 
	private int k=0;
	private static int cluster_num;
	
    public Pre_Cluster_process(String input_path, String reduce_number) {
		// TODO Auto-generated constructor stub
    	this.input_path=input_path; 
    	this.k=Integer.parseInt(reduce_number);
	}
  
    
    void Cluster_NJ_method ( ArrayList<String> cluster_list , String output,int k) throws IOException{ // the NJ method to cluster
    	...
    	ClusterFromMatrix(distance,output,cluster_list,cluster_list_size, k);
    }
    
    public static void ClusterFromMatrix(double [][] distance,String output,ArrayList<String> cluster_list , int cluster_list_size,int k) throws IOException{
    	Comparator<Point> comparator = new Comparator<Point>(){  
  		  public int compare(Point obj1,Point obj2){  // for the sort 
  			Point test1  = (Point)obj1;
  			Point test2  = (Point)obj2;
  				 if(test1.meature > test2.meature){
  					 return 1;
  				 }
  				 else if(test1.meature < test2.meature){
  					 return -1;
  				 }
  				 else {
  					 return 0;
    	...
    	ArrayList<Point> distance_list = new ArrayList<Point>(); 
    	...
    	System.out.println("\nthe init cluster number is : "+Sum_set.size()+"\n");
    	
    	/*
    	 * This should have the on load balance to the sum_set to make the set size==k  
    	...
    	if (Sum_set.size()>3*k){
        	...
        	System.out.println("k<Sum_set.size()<3k the class number is "+Sum_set.size());
        	cluster_num = Sum_set.size();
    	}// end   k<Sum_set.size()<=3*k
    		...
    		// now the Root_set.size > k 遍历每棵子树，写出聚类信息
    		int label=0;
    		...
    		System.out.println("Sum_set.size()<k the class number is "+Root_set.size());
    ...
    public static void CountTheDistance(ArrayList<String>sequences,int lineNumber,double[][] distance,double[] A) {//这个函数用来计算距离矩阵
		int i,j;
		double p;
		double temp;
    ...
    public void Get_Cluster(String output, int line_number) throws IOException{
    	...
    	ArrayList<String> cluster_list = new ArrayList<String>();//用于保存样本序列
    	ArrayList<Integer> random_list = new ArrayList<Integer>();  // the index of the samples
    	...
    	while(count < select_number){
    	    ...
    	    random_list.add(ran_num);  // add the random list
    		}
    	}    	
    	Collections.sort(random_list);
    	
        // get the random data to cluster
        BufferedReader br = new BufferedReader(new FileReader(input_path));   // the input data is key value format
        count=0;
        ...
        br.close();
        // now the data to cluster is store in the list cluster_list
        Cluster_NJ_method(cluster_list,output,k);
